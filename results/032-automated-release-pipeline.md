...
- `scripts/release_pipeline.py`를 추가해 workflow가 버전 조회, artifact 생성, checksum manifest 생성을 동일한 로컬 코드로 수행하게 했다.
...
- Artifact는 현재 Task 033의 launcher/updater가 소비할 metadata와 `bin/devbot --version` fixture를 포함하는 deterministic tarball로 구성했다.
...
| CP-032-2 release intent | `test_release_increment_is_selected_from_merged_pr_label`, `test_missing_or_conflicting_release_labels_fail_closed`, `test_release_none_skips_publication` |
...
| CP-032-6 artifacts | `test_release_artifact_names_are_deterministic`; workflow matrix covers macOS/Linux arm64/x86_64 |
...
| CP-032-10 release notes | `test_release_note_generation_is_deterministic`; sample: `- minor: #67 Task 032: Automated Release Pipeline` |
...
| CP-032-13 manual dispatch | workflow `workflow_dispatch` inputs `increment` and `commit_sha`; manual job rejects non-main commits |
...
- `UV_CACHE_DIR=/tmp/devbot-uv-cache uv run pytest`: PASS, 503 passed
- `WORKSPACE_ROOT=/tmp/devbot-task032-workspace ... uv run devbot --once --dry-run`: PASS, `no_managed_repositories`
- `uv run devbot doctor`: local constrained FAIL. 현재 `doctor`는 startup self-update에서 operator checkout의 `git fetch origin main`을 수행한다. 이번 Task prompt가 원격 discovery/network 명령을 금지했고 dummy token 환경에서는 인증 실패가 발생했다.
...
- `plan-release` job의 GitHub metadata 기반 자동 increment 산출은 현재 workflow fixture와 로컬 정책 helper가 검증하는 형태이며, 운영 환경에서 PR label 조회 wiring을 추가로 강화할 수 있다.
