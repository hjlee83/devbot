...
`uv sync`/`uv run ...`은 현재 sandbox에서 새 dependency download가 막혀 실패했다.
`UV_CACHE_DIR=/private/tmp/devbot-uv-cache uv run pytest ...` 실행 시 PyPI DNS lookup
실패로 `packaging`/`urllib3` 다운로드가 불가능했다. 대신 기존 host venv와
`PYTHONPATH=src`로 현재 worktree 코드를 검증했다.
