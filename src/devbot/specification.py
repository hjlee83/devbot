...
def render_specification(
    evidence: TaskEvidence, *, template_selection: TemplateSelection | None = None
) -> str:
    ...
    if template_selection is None:
        template_selection = select_specification_template(sections)
    template = template_selection.template
    template_policy = render_template_policy(template).rstrip()
    ...
    lines = [
        ...
        f"- Specification Template: `{template.id}` selected from `{template_selection.source}`.",
        "- Generated by `devbot specification generate` (Task 044) from repository "
        "evidence only - Goal, Issue, Task Contract, Roadmap, and deterministic "
        "template policy. No speculative content.",
        "",
        "# Overview",
        "",
        "## Goal",
...
def generate_specification(
    ...
    *,
    ...
    template_selection = select_specification_template(
        evidence.contract.sections, override=template_id
    )
    content = render_specification(evidence, template_selection=template_selection)
